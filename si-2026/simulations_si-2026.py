...
# --- Block Seat Calculations ---

print("\nCalculating block seat allocations...")

# Define the blocks
# Block 5: Levica/Vesna + SD + GS + PIR + PVP
# Block 6: Block 5 + DEM

# Map party names to their column names in the dataframe
# We need to identify which columns correspond to which parties
party_columns = seats_simulations_aging_cov.columns.tolist()
print(f"Available party columns: {party_columns}")

# Define block compositions - adjust these names to match actual column names
block_5_parties = ['Levica/Vesna', 'SD', 'GS', 'PIR', 'PVP']
block_6_parties = ['Levica/Vesna', 'SD', 'GS', 'PIR', 'PVP', 'DEM']

# Filter to only include parties that exist in the dataframe
block_5_parties_available = [p for p in block_5_parties if p in party_columns]
block_6_parties_available = [p for p in block_6_parties if p in party_columns]

print(f"Block 5 parties found: {block_5_parties_available}")
print(f"Block 6 parties found: {block_6_parties_available}")

# Calculate block seats for each simulation
blocks_simulations_aging_cov = pd.DataFrame(0, index=range(sample), columns=['Block 5', 'Block 6'], dtype=int)

for i in range(sample):
    # Block 5: Sum seats of all parties in Block 5
    block_5_seats = 0
    for party in block_5_parties_available:
        block_5_seats += seats_simulations_aging_cov.iloc[i][party]
    
    # Block 6: Sum seats of all parties in Block 6
    block_6_seats = 0
    for party in block_6_parties_available:
        block_6_seats += seats_simulations_aging_cov.iloc[i][party]
    
    blocks_simulations_aging_cov.iloc[i] = [block_5_seats, block_6_seats]

# Calculate probability distribution for each block
# Pr[S=x] for x from 0 to 88 seats
blocks_prob_aging_cov = pd.DataFrame(index=range(0, seat_max + 1), columns=['Block 5', 'Block 6'], dtype=float)

for block in blocks_simulations_aging_cov.columns:
    counts = blocks_simulations_aging_cov[block].value_counts(sort=False)
    for x in range(0, seat_max + 1):
        blocks_prob_aging_cov.loc[x, block] = counts.get(x, 0) / sample

# Write to Google Sheets
wsw = sh.worksheet('blocks_seats')
header = ['Pr[S=x]'] + blocks_prob_aging_cov.columns.values.tolist()
table = [header] + [[x] + blocks_prob_aging_cov.loc[x].values.tolist() for x in blocks_prob_aging_cov.index]
wsw.update(values=table, range_name='A1')

print(f"Block seat allocation complete. Results written to 'blocks_seats' worksheet.")
