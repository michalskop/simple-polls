...
# list of elections
url = "https://docs.google.com/spreadsheets/d/e/2PACX-1vT7hw4mzvUhf0k0Ed6AQ_-bs5b5JhLT_6VRdUnbwlOjGA4GlDMN3iG_XdTOUXrmhqJ0SDO85RM4TV6z/pub?gid=0&single=true&output=csv"
# load the list
df = pd.read_csv(url)
df = df[~df['iso'].isna()]
...
# europe elects data
key = "1cua2Ymw3bb1UUPPRtGoniaLr2SMzAUDT5-S_fTkq0SI"
# connect to Google Sheets
gc = gspread.service_account(path + "secret/credentials.json")
shee = gc.open_by_key(key)
...
for i in range(len(df)):
  ...
  election_code = election['iso'] + "-eu-2024"
  election_flag = countryflag.getflag([election['iso']]) + '🇪🇺'
  election_date = election['date']
  source_election_code = "fi2024" # to copy from
  wikipedia_link = ""

  # get europe elects data for the country
  try:
    data_ee = shee.worksheet(election['iso'] + '_E').get_all_records()
  except:
    data_ee = shee.worksheet(election['iso']).get_all_records()

  # get the candidates, >= 1.5% or >= threshold and > 0.1
  threshold = max(min(election['threshold'], 1.5), 0.1)

  # find the candidates, starting after ''
  empty_n = list(data_ee[0].keys()).index('')
  keys_ee = list(data_ee[0].keys())[empty_n + 1:]
  candidates = []
  candidates_colors = []
  candidates_values = []
  candidates_needs = []
  for c in keys_ee:
    if data_ee[0][c] > threshold:
      candidates.append(c)
      candidates_colors.append('#BBBBBB')
      candidates_values.append(data_ee[0][c])
      candidates_needs.append(election['threshold'])
  ...
  # Create a new GSheet:
  sh = gc.create("Simulace - " + election_code_pretty)
  sh.id
  # add the sheet to the list
  urls.append("https://docs.google.com/spreadsheets/d/" + sh.id + "/edit")
  ...
  with open (path + election_code + '/simulations_' + election_code + '.py', 'w') as f:
    f.write(content)
  print(election_code, "Simulations file created.")
  time.sleep(60)
