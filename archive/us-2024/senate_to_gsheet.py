...
for state in states:
  try:
    ...
    cols_index = ["poll_id", "start_date", "end_date", "middle_date", "pollster", "sponsors", "population", "sample_size", "numeric_grade", "url", "pollscore", "transparency_score"]
    df3[cols_index] = df3[cols_index].fillna('NaN')
    df3_2[cols_index] = df3_2[cols_index].fillna('NaN')
    pt2 = pd.pivot_table(df3, values="pct", index=cols_index, columns=["answer"], aggfunc="mean").reset_index().replace('NaN', np.nan)
    pt2_2 = pd.pivot_table(df3_2, values="pct", index=cols_index, columns=["answer"]).reset_index().replace('NaN', np.nan)
    ...
    pt2all.fillna('NaN', inplace=True)
    ...
    pt2all_grouped.replace('NaN', np.nan, inplace=True)
